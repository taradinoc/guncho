...
namespace Guncho.Api
{
    ...
    public sealed class WebApiStartup
    {
        ...
        {
            // Serve Blazor client from wwwroot directory in output folder
            var blazorWwwrootPath = System.IO.Path.Combine(
                System.AppDomain.CurrentDomain.BaseDirectory,
                "wwwroot");
            
            if (System.IO.Directory.Exists(blazorWwwrootPath))
            {
                var blazorFileSystem = new Microsoft.Owin.FileSystems.PhysicalFileSystem(blazorWwwrootPath);
                appBuilder.UseDefaultFiles(new DefaultFilesOptions
                {
                    FileSystem = blazorFileSystem
                });
                appBuilder.UseStaticFiles(new StaticFileOptions
                {
                    FileSystem = blazorFileSystem,
                    // Serve Blazor framework files with correct MIME types
                    ContentTypeProvider = new Microsoft.Owin.StaticFiles.ContentTypes.FileExtensionContentTypeProvider(new Dictionary<string, string>
                    {
            ...
            // TODO: Optionally serve Angular client from embedded resources for legacy compatibility
